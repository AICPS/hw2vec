...
def parse_single_verilog_code(verilog_path, output_path, graph_format="DFG"):
    ...
    # be flexible to different encoding technique
    # Agraph generated by graphGenerator is not accepted in pytorch-geometric
    hardware_graph = nx.DiGraph() # https://pytorch-geometric.readthedocs.io/en/latest/_modules/torch_geometric/utils/convert.html
    for src in graph_json['edge_index']:
        hardware_graph.add_node(src, label=label2idx[src.split(".")[-1].split("_")[0].lower()])
        for neighbor in graph_json['edge_index'][src]:
            dst = neighbor[1]
            hardware_graph.add_edge(src, dst)
    
    # edge idx is in data.edge_index
    data = from_networkx(hardware_graph)
    X = F.one_hot(torch.LongTensor(data.label), num_classes=len(label2idx)).float()
