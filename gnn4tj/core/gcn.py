...
class GCN(nn.Module):
    ...
    def forward(self, x, edge_index, batch=None):
        ...
        if self.pooling_type == "sagpool":
            x, edge_index, _, attn_weights['batch'], attn_weights['pool_perm'], attn_weights['pool_score'] = self.pool1(x, edge_index, batch=batch)
        elif self.pooling_type == "topk":
            x, edge_index, _, attn_weights['batch'], attn_weights['pool_perm'], attn_weights['pool_score'] = self.pool1(x, edge_index, batch=batch)
        elif self.pooling_type == "asa":
            x, edge_index, _, attn_weights['batch'], attn_weights['pool_perm'] = self.pool1(x, edge_index, batch=batch)
        else: 
        ...
        if self.readout_type == "add":
            ...
            x = global_sort_pool(x, attn_weights['batch'], k=100)
        else:
            pass
